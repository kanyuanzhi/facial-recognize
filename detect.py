...
@torch.no_grad()
def run(data_queue, weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    top, right, bottom, left = int(xyxy[1]), int(xyxy[2]), int(xyxy[3]), int(xyxy[0])
                    face_locations.append((top, right, bottom, left))
                # im0c = im0.copy()
                # im0c_resize = cv2.resize(im0c, (0, 0), fx=0.5, fy=0.5)
                ...
                recognition_results = [(pred, loc) if rec else ("unknown", loc) for pred, loc, rec in zip(knn_clf.predict(face_encodings), face_locations, are_matches)]
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        label = recognition_results[label_index][0]
                        label_index += 1
            ...
            data_queue.put((im0, 'text_bytes'))

        t5 = time_sync()
        dt[3] += t5 - t4

        # Print time (inference-only)
        # LOGGER.info(f'{s}Done. ({t5 - t2:.3f}s)')
...
def main(data_queue, opt):
    check_requirements(exclude=('tensorboard', 'thop'))
    run(data_queue, **vars(opt))
...
def send_data(data_queue):
    ...
    while True:
        (frame, text) = data_queue.get()
        client.send('123', frame, text)


if __name__ == "__main__":
    data_queue = Manager().Queue()
    send_data_process = Process(target=send_data, args=(data_queue,))
